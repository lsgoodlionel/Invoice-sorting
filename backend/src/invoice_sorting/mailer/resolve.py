...
def resolve_mail(settings: Settings, control: Session) -> ResolvedMail:
    ...
    if env is not None:
        return ResolvedMail(source=SOURCE_ENV, config=env, base_url=settings.public_base_url)
    ...
    if row is None or not is_row_configured(row):
        return ResolvedMail(source=SOURCE_NONE, config=None, base_url=web_base)
    return ResolvedMail(source=SOURCE_WEB, config=web_config(settings, row), base_url=web_base)
