...
任务状态存在进程内存里（重启后作废，重新导出即可）。包的存放位置由调用方决定：
平台后台替某账套导出落在平台根目录 `备份/租户导出/`（只保留最近 KEEP_EXPORTS 个）；
账套自助备份/导出落在该账套自己的 `备份/账本备份/`（见 ledger_backups，保留最近 10 份）。
"""
...
def exports_dir(settings: Settings) -> Path:
    """平台侧与命令行默认的搬迁包输出目录（部署根目录下的备份区，不随单个租户目录被覆盖）。"""
