...
def _grant(request: Request, response: Response, grant: Grant) -> dict[str, Any]:
    set_session_cookie(response, grant.token, is_secure_request(request))
    return ok({"authenticated": True, "user": grant.user.as_dict()})
...
def auth_status(
    ...
    if config.auth_enabled:
        ...
        user = check.user.as_dict() if authenticated and check.user else None
...
def auth_setup(
    ...
    grant = setup_initial_password(session, body.password, _user_agent(request))
    return _grant(request, response, grant)
...
def auth_login(
    ...
    grant = login_with_password(session, body.username, body.password, _user_agent(request))
    if grant is None:
...
def auth_change_password(
    body: ChangePasswordBody, request: Request, session: SessionDep, user: CurrentUserDep
) -> dict[str, Any]:
    if user is None:
        raise AppError(MSG_AUTH_DISABLED, status_code=400)
    token = read_session_token(request)
    change_password(session, user.id, body.current_password, body.new_password, token)
    return ok(None)
